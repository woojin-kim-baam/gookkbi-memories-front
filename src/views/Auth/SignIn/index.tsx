...
export default function SignIn(props: Props) {
  ...
  // state: cookie 상태 //
  const [_, setCookie] = useCookies(); // _ : 쓰지 않겠다는
  ...
  const signInResponse = (responseBody: SignInResponseDto | ResponseDto | null) => {
    ...
    // 왜 APP.tsx에서는 상대경로를 쓰고 여기서는 절대경로를 쓰는가?
    // 수정할때는 가만했는데, 실제 사용했을때는 절대경로를 쓰는데, 편의성을 위해.
  };
    ...
    <div id="auth-login-container">
      ...
      <div className="sns-container">
        ...
        <div className="sns-button-box">
          <div className="sns-button kakao" onClick={() => onSnsButtonClickHanlder("kakao")}></div>
          <div className="sns-button naver" onClick={() => onSnsButtonClickHanlder("naver")}></div>
        </div>
      </div>
    </div>
  );
