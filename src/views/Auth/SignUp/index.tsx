...
export default function SignUp(props: Props) {
  ...
  const isSns = joinType !== "NORMAL" && SNS_ID !== undefined;
  ...
  const inCheckResponse = (responseBody: ResponseDto | null) => {
    ...
    // const isSuccess = responseBody?.code === "SU"; // 이 물음표는 responseBody는 null 값을 가질 수 있기에
  ...
  const onCheckUserIdClickHandler = () => {
    ...
    const requestBody: IdCheckRequestDto = {
      userId // 원래 : userId 써야하는데 변수명과 속성명이 동일하기에 생략 가능
    };

    idCheckRequest(requestBody).then(inCheckResponse);
  };
        ...
        <div className="sns-container">
          ...
          <div className="sns-button-box">
            <div
              ...
              onClick={() => onSnsButtonClickHanlder("kakao")}
            ></div>
            <div
              className="sns-button naver"
              onClick={() => onSnsButtonClickHanlder("naver")}
            ></div>
          </div>
        </div>
      )}
