...
function UserUpdate({onModalViewChange} : UserUpdateProps) {
  ...
  const onAgeChangeHandler = (event: ChangeEvent<HTMLInputElement>) => {
    const { value } = event.target
    const regexp = /^$|^[1-9][0-9]{0,2}$/  // ^$이게 빈문자열, 띄어쓰기 하면안돼!!
  ...
  // event handler: 사용자 주소 검색 버튼 클릭 이벤트 처리 //
  ...
  return (
    <div className='user-update-container'>
      ...
      <InputBox label='상세 주소' value={updateDetailAddress} placeholder='상세주소를 입력해주세요' type='text' message='' onChange={ondetailAddressChangeHandler}/>
...
export default function UserInfo() {
  ...
  return (
    <div className='user-info-container'>
        ...
        <div className='button second middle' onClick={onUpdateOpenButtonClick}>수정</div>
        {isUpdateOpen && 
        <Modal 
        title='회원 정보 수정' 
        onClose={onUpdateOpenButtonClick}
        >
          <UserUpdate onModalViewChange={onUpdateOpenButtonClick}/>
        </Modal>}
    </div>
  )
}
