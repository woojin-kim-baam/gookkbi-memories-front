...
export default function MemoryTest() {
   ...
   const postMemoryResponse = (responseBody: ResponseDto | null) => {
    ...
    //event handler: 검사 시작 버튼 클릭 이벤트 핸들러 //
    const onStartClickHandler = () => {
      setStarted(true);
      
    }

    //event handler: 카트 클릭 이벤트 핸들러 //
    const onCardClickHandler = (id: number) => {
      const selectedCard = memoryCards.find(card => card.id === id); // 그 순간에 선택된 하나의 녀석
      if(selectedCards.length === 2 || !selectedCard) return; // 지금까지 선택된 녀석
        ...
        <div className='container'>
            ...
            <div className='test-box'>
                ...
                <div className='card-container'>
                  {memoryCards.map((memoryCard, index) => <Card key={index} memoryCard={memoryCard} onClick={onCardClickHandler} />)}
                </div> :  
